...
high_scoreboard_obj = HighScoreBoard()
...
def reset_game(screen):
    global snake_obj
    global food_obj
    global scoreboard_obj
    if scoreboard_obj.score >= high_scoreboard_obj.high_score:
        high_scoreboard_obj.update_score(scoreboard_obj.score)
    for segment in snake_obj.snake:
        segment.reset()
    snake_obj.__init__()
    food_obj.reset()
    food_obj.__init__(screen)
    scoreboard_obj.reset()
    scoreboard_obj.__init__()
...
while game_is_on:
    ...
    time.sleep(.1)
    snake_obj.move()
    # Collision with wall or with tail
    if wall_collision(snake_obj) or tail_collision(snake_obj):
        reset_game(screen)
    # Collision with food
